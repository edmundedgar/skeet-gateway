...
import {SkeetGateway} from "../../SkeetGateway.sol";

contract SafeAddOwnerMessageParser is IMessageParser {
    function parseMessage(bytes[] calldata content, uint256 messageStart, uint256 messageEnd, bytes32 account)
        external
        view
        returns (address, uint256 value, bytes memory)
    {
        require(messageEnd - messageStart == 42, "Message should be 0x then 40 chars (20 bytes)");
        address signerSafe = SkeetGateway(msg.sender).selectedSafeAddress(account);
