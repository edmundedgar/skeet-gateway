...
contract SkeetGatewayTest is Test {
    ...
    function testActualSkeetBBSPost() public {
        ...
        address expectedSigner = gateway.predictSignerAddress(sha256(proof.commitNode), 28, proof.r, proof.s);

        assertNotEq(expectedSigner, address(0), "Signer not found");
        address expectedSafe = address(gateway.predictSafeAddress(sha256(proof.commitNode), 28, proof.r, proof.s));
